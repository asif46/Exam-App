...
import 'package:untitled/models/teacher_conduct_paper.dart';
...
class _ConductPaperState extends State<ConductPaper> {
  ...
  Future<List<ConductPaperModel>> getPaperList() async {
    ...
    var response = await http.get(
      Uri.https(BASE_URL, 'TSpanel/public/api/paper', _queryParameters),
    );
  ...
  updatePaperStatus(String status, paperId) async {
    ...
    var response = await http.put(
      Uri.https(
          BASE_URL, 'TSpanel/public/api/paper/edit/$paperId', _queryParameters),
    );
    if (response.statusCode == 200) {
      await getPaperList();
